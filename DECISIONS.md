...
- 质量循环中的错误与空路径属于产品界面的一部分：Admin 也必须使用与控制台一致的错误恢复层级，而不是遗留的开发期默认样式。
- 公开首页既是 CA 安检入口，也是 Web DApp 的发现入口；安全工作台必须在桌面导航和首屏行动区可见，不能要求用户猜测 `/app` 路由。
- 首页在桌面端使用双栏操作面板来表达 DApp 的并行任务，移动端仍按 CA 安检、工作台、风险说明的单列操作顺序呈现，避免为视觉而牺牲输入可用性。
- 共享移动导航根据当前路径自动选择公开站点或工作台信息架构，避免每个工作台页面手工维护底栏而产生跳出 DApp 的错误链接。
- 工作台导航改动须同时经过 Web 目标门禁、运行态 smoke 与全仓门禁，确保客户端路由状态不会破坏 API、Bot、Admin 或共享 package 的构建链路。
- Web 的 `@import "tailwindcss"` 必须由同包的 `@tailwindcss/postcss` 插件编译；样式构建配置属于可测试产品 contract，不能仅依赖 Next 构建成功来判断 UI 已渲染。
