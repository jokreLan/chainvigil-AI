...
- 开始开发 loop 第 106 轮：将公开首页从单列信息流调整为桌面安全工作区布局，同时保持移动端的紧凑单列阅读路径。
- 完成开发 loop 第 106 轮：首页在桌面端以 CA 安检、工作台入口、风险预警和防御模块组成双栏操作面板；VP 区域跨栏保留为统一的状态入口，核心模块补充悬浮反馈。
- 第 106 轮目标门禁通过：`@chainvigil/web` lint/typecheck、test（6 个用例）和 production build 通过；首页运行态 HTTP 200。
- 开始开发 loop 第 107 轮：修复移动端进入 Web DApp 后仍展示公开站点导航的上下文断层。
