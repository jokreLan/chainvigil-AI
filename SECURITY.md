...
请不要为未修复的安全漏洞创建公开 Issue，也不要在公开讨论中附带密钥、用户数据或可直接利用的攻击细节。

请通过 GitHub 仓库的 **Security → Report a vulnerability** 私下提交报告，并包含：
...
维护者会尽力确认报告、评估影响并协调修复，但实验性项目不承诺固定响应时效或商业 SLA。
