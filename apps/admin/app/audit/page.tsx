...
export default async function AdminAuditPage() {
  ...
  return (
    <main className="min-h-screen px-5 py-8 md:px-8 md:py-10">
      <p className="text-sm font-semibold text-[#c0c1ff]">AUDIT TRAIL</p>
      <h1 className="mt-2 text-3xl font-semibold text-white">后台审计日志</h1>
      ...
      <section className="mt-8 overflow-hidden rounded-lg border border-[#262932] bg-[#16181d]">
        ...
        <div className="divide-y divide-[#262932]">
          {auditLogs.map((log) => (
            <article key={log.id} className="grid gap-4 px-4 py-4 md:grid-cols-[10rem_1fr_12rem]">
              <div>
                <p className="text-sm font-semibold text-[#c0c1ff]">{actionLabels[log.action]}</p>
                <p className="mt-1 text-xs text-[#8f8b9e]">{log.action}</p>
              </div>
              <div>
                <p className="break-all text-sm text-[#e4e1ed]">{log.target}</p>
                <p className="mt-2 text-sm text-[#c7c4d7]">{log.reason}</p>
                {log.metadata ? (
                  <pre className="mt-3 overflow-x-auto rounded border border-[#363944] bg-[#0f1015] p-3 text-xs leading-5 text-[#c7c4d7]">
