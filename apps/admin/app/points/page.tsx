...
export const metadata = { title: "Vigil Points 审核" };
...
export default function AdminPointsPage() {
  ...
  return (
    <main className="admin-page min-h-screen">
      <p className="admin-kicker text-[#eab308]">VIGIL POINTS · MOCK LEDGER</p>
      <h1 className="admin-title">哨点 VP 事件审核</h1>
      <p className="admin-lead">
        高价值贡献和推广奖励必须支持 pending / confirmed / rejected。
      </p>
      <section className="admin-panel mt-8 p-5">
        ...
        <dl className="mt-5 grid gap-3 text-sm md:grid-cols-3">
          <div className="rounded border border-[#363944] p-3">
            ...
            <dd className="mt-1 text-lg font-semibold text-[#eab308]">
              {ledger.totalConfirmed} VP
            </dd>
          </div>
          <div className="rounded border border-[#363944] p-3">
            <dt className="text-[#8f8b9e]">待确认</dt>
            <dd className="mt-1 text-lg font-semibold text-[#fde68a]">
      ...
      <div className="mt-8 space-y-3">
        {ledger.recentEvents.map((event) => (
          <div key={event.id} className="admin-panel p-4">
