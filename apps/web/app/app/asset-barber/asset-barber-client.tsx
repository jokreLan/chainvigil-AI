...
const blockedAssets = [
  {
    ...
    tag: "Honeypot",
    address: "0x1234...abcd",
    copy: "Interacting with this contract may drain your wallet. Leave it alone.",
    tone: "#EF4444",
  },
  {
    icon: "⚠",
    ...
    tag: "High Risk",
    address: "0x9876...def0",
    copy: "Malicious code detected in transfer function.",
    tone: "#F97316",
  },
];
...
export function AssetBarberClient() {
  ...
  useEffect(() => {
    ...
    const timers = scanSteps.map((_, index) =>
      window.setTimeout(() => setStep(index + 1), (index + 1) * 650),
    );
    const finish = window.setTimeout(
      () => setScreen(profile === "failed" ? "failed" : "result"),
      3100,
    );
    return () => {
  ...
  return (
    <main className="cv-workspace-page min-h-screen pb-28 text-[#e4e1ed] selection:bg-[#8083ff] selection:text-[#0d0096] md:pb-8">
      <WorkspaceHeader desktopActive="wallets" />
      <div className="mx-auto max-w-[1280px] px-4 pt-8 md:px-6 md:pt-10">
        {screen === "setup" && (
          <Setup
            address={address}
            scope={scope}
    ...
    <div className="space-y-8">
      <section className="space-y-2">
        <p className="inline-flex rounded-full border border-[#f59e0b]/40 bg-[#f59e0b]/10 px-3 py-1 text-xs font-semibold text-[#fde68a]">
          {isZh
            ? "演示模式 · 不读取真实钱包、不执行清理"
            : "Demo mode · no live wallet reads or cleanup"}
        </p>
      ...
      <section className="grid gap-4 md:grid-cols-12">
        <article className="relative flex min-h-[200px] flex-col items-center justify-center rounded-xl border border-[#262932] bg-[#16181D] p-6 shadow-lg shadow-black/20 md:col-span-4">
          <p className="mb-2 text-xs font-semibold uppercase tracking-[0.05em] text-[#c7c4d7]">
            Wallet Cleanliness Score
          </p>
          <div className="relative flex size-32 items-center justify-center">
            ...
            >
              ...
              <circle
                ...
                strokeDashoffset={62.19}
                strokeLinecap="round"
                strokeWidth="8"
              />
            </svg>
            <div className="relative z-10 text-center">
              <span className="block text-4xl font-bold text-[#F59E0B]">
                {profileScore(report.cleanliness)}
              </span>
          ...
          <p className="mt-4 text-xs font-semibold text-[#F59E0B]">
            {report.cleanliness >= 90 ? "Clean" : "Needs Attention"}
          </p>
        </article>
        ...
        <div className="grid grid-cols-2 gap-4 md:col-span-8 lg:grid-cols-4">
          {report.metrics.slice(0, 4).map((metric, index) => (
            <article
              key={metric.label}
              className="flex min-h-[150px] flex-col justify-between rounded-xl border border-[#262932] bg-[#16181D] p-5 transition hover:border-[#908fa0]"
            >
              <div className="flex items-start justify-between gap-2">
                <span className="text-2xl" style={{ color: metric.tone }}>
                  {["⚠", "♻", "⌫", "▧"][index]}
                </span>
                <span
                  className="rounded bg-white/5 px-2 py-1 text-xs font-semibold"
              ...
              <div className="mt-4">
                <h2 className="mb-1 text-xs font-semibold uppercase tracking-[0.05em] text-[#c7c4d7]">
                  {metric.label}
                </h2>
      ...
      <section className="relative overflow-hidden rounded-xl border border-[#c0c1ff]/20 bg-[#1b1b23] p-6">
        <div className="relative z-10 flex items-start gap-4">
          <span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-[#8083ff] text-xl text-[#0d0096]">
            ▣
          </span>
          <div>
            <h2 className="mb-2 text-2xl font-semibold text-[#e1e0ff]">
              AI Cleanup Strategy Recommended
            </h2>
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              <button
                type="button"
                onClick={onScan}
                className="flex min-h-12 items-center gap-2 rounded-lg bg-[#c0c1ff] px-6 text-sm font-semibold text-[#1000a9] shadow-lg shadow-[#c0c1ff]/20 transition hover:opacity-90"
              >
                <span>⌖</span>
              ...
              <button
                type="button"
                onClick={onUpgrade}
                className="min-h-12 rounded-lg border border-[#464554] px-5 text-sm font-semibold text-[#c7c4d7] transition hover:border-[#c0c1ff]"
              >
      ...
      <section className="grid gap-4 lg:grid-cols-[1fr_1fr]">
        ...
        <article className="rounded-xl border border-[#262932] bg-[#16181D] p-5">
          ...
          <h2 className="mt-1 text-2xl font-semibold text-[#F9FAFB]">
            {isZh
              ? "先处理授权，别为粉尘多付成本"
              : "Fix approvals first — don’t overpay for dust"}
          </h2>
            ...
            {report.actions.length ? (
              report.actions.map((action) => (
                <button
                  ...
                  key={action.id}
                  className="flex w-full items-center justify-between gap-4 rounded-lg border border-[#34343d] bg-[#1f1f27] p-4 text-left transition hover:border-[#8083ff]"
                >
                  <span>
                    <strong className="block text-[#F9FAFB]">
                      {action.title}
                    </strong>
                  ...
                  <span
                    className={
                      action.tone === "danger"
                        ? "shrink-0 text-sm font-semibold text-[#EF4444]"
                        : action.tone === "warning"
                          ? "shrink-0 text-sm font-semibold text-[#F59E0B]"
                          : "shrink-0 text-sm font-semibold text-[#10B981]"
                    }
                  >
        ...
        <article className="flex flex-col overflow-hidden rounded-xl border border-[#262932] bg-[#16181D]">
          ...
          <div className="grow space-y-3 p-4">
            {recoverable.map((item) => (
              <div
                key={item.symbol}
                className="flex items-center justify-between rounded-lg bg-[#1f1f27] p-3 transition hover:outline hover:outline-1 hover:outline-[#c0c1ff]/30"
              >
                <div className="flex items-center gap-3">
                  <span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-[#34343d] text-xs font-bold text-[#c7c4d7]">
                    {item.symbol}
                  </span>
                  <span>
                    <strong className="block text-xs font-semibold text-[#F9FAFB]">
                      {item.name}
                    </strong>
        ...
        <article className="flex flex-col overflow-hidden rounded-xl border border-[#262932] bg-[#16181D]">
          ...
          <div className="grow space-y-3 p-4">
            {hiddenAssets.map((item) => (
              <div
                key={item.name}
                className="flex items-center justify-between rounded-lg bg-[#1f1f27] p-3 transition hover:outline hover:outline-1 hover:outline-[#F59E0B]/30"
              >
                <div className="flex items-center gap-3">
                  <span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-[#34343d] text-sm text-[#F59E0B]">
                    {item.icon}
                  </span>
                  <span>
                    <strong
                      className="block max-w-28 truncate text-xs font-semibold text-[#F9FAFB]"
                      title={item.name}
                    >
                      {item.name}
                    </strong>
        ...
        <article className="relative flex flex-col overflow-hidden rounded-xl border border-[#EF4444]/30 bg-[#16181D]">
          ...
          <div className="grow space-y-3 p-4">
            {blockedAssets.map((item) => (
              <div
                key={item.name}
                className="relative overflow-hidden rounded-lg border border-[#EF4444]/20 bg-[#1f1f27] p-3"
              >
                <div className="mb-2 flex items-start justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <span style={{ color: item.tone }}>{item.icon}</span>
                    <strong className="text-xs font-semibold text-[#F9FAFB]">
                      {item.name}
                    </strong>
                  </div>
                  <span className="rounded bg-[#EF4444]/20 px-1.5 py-0.5 text-[10px] font-bold uppercase text-[#EF4444]">
                    {item.tag}
                  </span>
                </div>
                <p className="truncate font-mono text-xs text-[#c7c4d7]">
                  {item.address}
                </p>
                <p className="mt-2 text-xs leading-tight text-[#c7c4d7]">
                  {item.copy}
                </p>
              </div>
            ))}
...
function CategoryHeader({
  ...
  return (
    <div
      className={
        danger
          ? "flex items-center justify-between border-b border-[#EF4444]/20 bg-[#93000a]/20 p-4"
          : "flex items-center justify-between border-b border-[#262932] bg-[#393841] p-4"
      }
    >
      <h3
        className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.05em]"
        style={{ color: tone }}
      >
        <span>{icon}</span>
        {title}
      ...
      <span
        className="rounded border border-current/20 bg-[#16181D] px-2 py-0.5 text-xs font-semibold"
        style={{ color: danger ? tone : "#c7c4d7" }}
      >
        {count}
      </span>
    </div>
    ...
    <div className="flex flex-wrap gap-2">
      {(["all", "sol", "bnb"] as ChainScope[]).map((item) => (
        <button
          ...
          className={
            scope === item
              ? "min-h-11 rounded-lg bg-[#8083ff] px-4 py-2 text-sm font-semibold text-[#1000a9]"
              : "min-h-11 rounded-lg border border-[#464554] px-4 py-2 text-sm text-[#c7c4d7]"
          }
        >
          {item === "all"
            ? "SOL + BNB"
  ...
  return (
    <section className="mx-auto max-w-[780px] space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex min-h-11 items-center text-sm font-semibold text-[#c0c1ff]"
      >
        ← Back to dashboard
      </button>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="w-full">
            ...
            <h2 className="mt-2 text-2xl font-semibold text-[#F9FAFB]">
              {isZh ? "输入公开演示地址" : "Paste a public demo address"}
            </h2>
            <p className="mt-2 leading-7 text-[#c7c4d7]">
              {isZh
                ? "选择扫描范围后，使用本地 Mock 数据生成完整报告。无需连接钱包，也不会请求签名。"
                : "After choosing scope, local mock data builds the full report. No wallet connection or signature is requested."}
            </p>
            <div className="mt-5">
              <ScopeTabs scope={scope} onScope={onScope} />
            </div>
            <label
              htmlFor="asset-barber-address"
              className="mt-6 block text-sm font-semibold text-[#e4e1ed]"
            >
              {isZh ? "钱包地址" : "Wallet address"}
            </label>
            <input
              id="asset-barber-address"
              type="text"
              value={address}
              ...
              aria-describedby={
                error ? "asset-barber-address-error" : undefined
              }
              autoCapitalize="off"
              autoComplete="off"
              autoCorrect="off"
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {(["risk", "clean", "empty", "failed"] as ScanProfile[]).map(
                (item) => (
                  <button
                    key={item}
                    type="button"
                    ...
                    className={
                      profile === item
                        ? "min-h-11 rounded-full border border-[#c0c1ff] bg-[#8083ff]/20 px-3 py-1.5 text-xs font-semibold text-[#e1e0ff]"
                        : "min-h-11 rounded-full border border-[#464554] px-3 py-1.5 text-xs text-[#c7c4d7]"
                    }
                  >
                    {isZh ? "演示：" : "Demo: "}
                    {profileLabels[item]}
            ...
            {error && (
              <p
                id="asset-barber-address-error"
                role="alert"
                className="mt-3 text-sm text-[#ffb4ab]"
              >
                {error}
              </p>
            )}
            <div className="mt-6">
              <button
                type="button"
                onClick={onScan}
                className="min-h-12 w-full rounded-lg bg-[#c0c1ff] px-5 font-semibold text-[#1000a9] shadow-lg shadow-[#c0c1ff]/20 transition hover:opacity-90"
              >
  ...
  return (
    <section className="mx-auto max-w-[780px] space-y-6">
      <div className="space-y-2">
        ...
        <p className="break-all text-lg leading-8 text-[#c7c4d7]">
          {address} ·{" "}
          {scope === "all"
            ? "SOL + BNB"
            : scope === "sol"
      ...
      <article className="rounded-xl border border-[#262932] bg-[#16181D] p-6 shadow-lg shadow-black/20">
        <div className="flex items-start gap-4">
          <span className="flex size-10 shrink-0 animate-pulse items-center justify-center rounded-lg bg-[#8083ff] text-[#0d0096]">
            ▣
          </span>
          <div className="w-full">
            <p className="text-xs font-semibold uppercase tracking-[0.05em] text-[#c0c1ff]">
            ...
            <ol className="mt-6 space-y-3">
              {steps.map((item, index) => (
                <li
                  key={item}
                  className="flex items-center gap-3 rounded-lg border border-[#262932] bg-[#1f1f27] px-4 py-4"
                >
                  <span
                    className={
                      index < step
                        ? "flex size-6 items-center justify-center rounded-full bg-[#10B981] text-xs font-bold text-[#07110d]"
                        : index === step
                          ? "flex size-6 animate-pulse items-center justify-center rounded-full bg-[#8083ff] text-xs font-bold text-[#1000a9]"
                          : "flex size-6 items-center justify-center rounded-full bg-[#34343d] text-xs text-[#c7c4d7]"
                    }
                  >
                    {index < step ? "✓" : index + 1}
                  </span>
                  <span
                    className={
                      index <= step ? "text-[#F9FAFB]" : "text-[#9CA3AF]"
                    }
                  >
                    {item}
                  </span>
            ...
            <div className="mt-6 h-2 overflow-hidden rounded-full bg-[#34343d]">
              <div
                ...
                style={{
                  width: `${Math.max(8, (step / steps.length) * 100)}%`,
                }}
              />
            </div>
          </div>
...
function Failed({
  ...
  return (
    <section className="mx-auto max-w-[780px] rounded-xl border border-[#EF4444]/35 bg-[#16181D] p-6">
      <p className="text-xs font-semibold uppercase tracking-[0.05em] text-[#ffb4ab]">
        Scan failed · mock state
      </p>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        <button
          type="button"
          onClick={onRetry}
          className="min-h-12 rounded-lg bg-[#ffb4ab] px-6 font-semibold text-[#690005]"
        >
        ...
        <button
          type="button"
          onClick={onBack}
          className="min-h-12 rounded-lg border border-[#464554] px-6 font-semibold text-[#e4e1ed]"
        >
    ...
    <div className="rounded-lg border border-[#262932] bg-[#1f1f27] p-4">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        {(
          [
            [
              isZh ? "空 Token Account" : "Empty token accounts",
              report.sol.emptyAccounts,
            ],
            [
              isZh ? "预计可回收" : "Est. recoverable",
              report.sol.recoverableSol,
            ],
      ...
      <div className="mt-4 space-y-2">
        {report.sol.dust.length ? (
          report.sol.dust.map((item) => (
            <div
              key={item.name}
              className="grid gap-2 rounded-lg border border-[#34343d] bg-[#16181D] p-3 text-sm sm:grid-cols-[1fr_.8fr_1fr_1fr]"
            >
          ...
          <div className="rounded-lg border border-dashed border-[#464554] p-4 text-[#9CA3AF]">
            {isZh
              ? "没有发现 SOL 粉尘资产或垃圾 NFT。"
              : "No SOL dust or spam NFTs found."}
          </div>
        )}
      </div>
        ...
        <button
          ...
          onClick={() =>
            onAction({
              ...
              title: isZh
                ? "查看空账户清理模拟"
                : "Preview empty-account cleanup",
              detail: isZh
                ? "不会执行真实 CloseAccount。"
                : "Will not execute a real CloseAccount.",
              recommendation: isZh ? "需要确认" : "Confirm needed",
              tone: "warning",
            })
          }
      ...
      <div className="mt-4 space-y-2">
        {report.bnb.length ? (
          report.bnb.map((item) => (
            <div
              key={item.spender}
              className="rounded-lg border border-[#34343d] bg-[#16181D] p-3"
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                <strong className="font-mono text-[#F9FAFB]">
                  {item.spender}
                </strong>
                <span
                  className={
                    item.risk === "High"
                      ? "rounded bg-[#EF4444]/15 px-2 py-1 text-xs font-semibold text-[#ffb4ab]"
                      : item.risk === "Medium"
                        ? "rounded bg-[#F59E0B]/15 px-2 py-1 text-xs font-semibold text-[#F59E0B]"
                        : "rounded bg-[#10B981]/15 px-2 py-1 text-xs font-semibold text-[#10B981]"
                  }
                >
                  {item.risk}
                </span>
...
function SafetyModal({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="asset-barber-modal-title"
    ...
    >
      <div className="w-full max-w-lg rounded-xl border border-[#464554] bg-[#16181D] p-6 shadow-2xl">
        ...
        <h2
          id="asset-barber-modal-title"
          className="mt-2 text-2xl font-semibold text-[#F9FAFB]"
        >
          {isUpgrade
            ? isZh
              ? "Pro 功能仅为演示"
              : "Pro features are demo-only"
            : action?.title}
        </h2>
        <p
          id="asset-barber-modal-description"
          className="mt-4 leading-7 text-[#c7c4d7]"
        >
          {isUpgrade
            ? isZh
        ...
        <button
          type="button"
          onClick={onClose}
          className="mt-6 min-h-12 w-full rounded-lg bg-[#c0c1ff] font-semibold text-[#1000a9]"
        >
          {isZh ? "我知道了" : "Got it"}
        </button>
      </div>
