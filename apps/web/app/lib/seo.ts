...
const copy: Record<PageSeoKey, Record<Locale, { title: string; description: string }>> = {
  home: {
    zh: {
      title: "ChainVigil — 买币前，先查 CA",
      description:
        "中文买前安检入口：人话风险结论，优先 SOL/BNB。查 CA、群内 Bot、哨点 VP。不构成投资建议。",
    },
    en: {
      title: "ChainVigil — Before you buy, check the CA",
      description:
  ...
  fakeDb: {
    zh: {
      title: "假 USDT / USDC / Meme 仿盘合约对照库｜ChainVigil",
      description:
        "活跃仿盘与假稳定币模式对照：名称可伪造，买前务必核对唯一 CA。教育向样例，请独立复核。",
    },
    en: {
      title: "Active Fake USDT, USDC & Meme Impostor Contracts｜ChainVigil",
      description:
  ...
  bnb: {
    zh: {
      title: "BSC 貔貅扫描 · 撤销授权 · CA 安检｜ChainVigil",
      description:
        "BNB Smart Chain honeypot scanner 教育、Revoke approvals 说明与 CA 极速安检入口。买前查 CA，授权清理见工作台（V0 不广播交易）。",
    },
    en: {
      title: "BSC Honeypot Scanner, Revoke Approvals & CA Check｜ChainVigil",
      description:
    ...
    "@graph": [
      {
        ...
        name: isZh ? "ChainVigil" : "ChainVigil",
        url: site,
...
export function buildDevelopersJsonLd(locale: Locale = "zh") {
  ...
  return {
    ...
    "@graph": [
      {
        "@type": "TechArticle",
        headline: isZh ? "ChainVigil 开发者中心" : "ChainVigil Developers",
        description: isZh
        ...
        author: { "@type": "Organization", name: "ChainVigil" },
        publisher: { "@type": "Organization", name: "ChainVigil" },
        about: [
      ...
      },
      {
        ...
        provider: { "@type": "Organization", name: "ChainVigil" },
      },
      {
        "@type": "APIReference",
...
export function buildRiskDatasetJsonLd(locale: Locale = "zh") {
  ...
  return {
    ...
    creator: { "@type": "Organization", name: "ChainVigil", url: site },
    keywords: isZh
